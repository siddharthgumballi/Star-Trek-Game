...
# 1c = 299,792 km/s = 299.792 units/s (since 1 unit = 1000 km)
const LIGHT_SPEED_UNITS: float = 299.792  # units per second
...
func engage_warp() -> void:
	...
	if not is_safe:
		var distance_km: float = distance * 1000.0  # Convert to km
		var reason: String = "Too close to %s (%.0f km). Minimum safe distance: %.0f km" % [
			nearest_body, distance_km, min_safe_warp_distance * 1000.0
		]
